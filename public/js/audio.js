...
// so we pre-create and resume the context here while we're guaranteed to be in one.
function _unlock() {
    if (audioCtx) return;
    audioCtx = new (window.AudioContext || window.webkitAudioContext)();
    audioCtx.resume().then(() => _initGraph(audioCtx));
}
document.addEventListener('touchstart', _unlock, { once: true });
document.addEventListener('click',      _unlock, { once: true });

// Called from startExperience() as a fallback if the touch unlock hasn't fired yet.
...
export function setEarphonesMode(active) {
    ...
    // 0.6 active vs 0.55 inactive: the 0.05 difference gives a perceptible state change;
    // 0.55 is the minimum for outdoor legibility over a sunlit camera feed.
...
export function playExplosion(pos, shellType) {
    ...
    thumpOsc.connect(shaper); shaper.connect(thumpGain); thumpGain.connect(panner);
    thumpOsc.start(now); thumpOsc.stop(now + 0.35);
    ...
    bodySrc.connect(bodyFilter); bodyFilter.connect(bodyGain); bodyGain.connect(panner);
    bodySrc.start(now);
    ...
    sizzleSrc.connect(sizzleFilter); sizzleFilter.connect(sizzleGain); sizzleGain.connect(panner);
    sizzleSrc.start(now);
    ...
    if (shellType === 2) {
        for (let k = 0; k < 6; k++) {
            ...
            crackleSrc.connect(crackleGain); crackleGain.connect(panner);
            crackleSrc.start(now + delay);
        }
    }

    // Release panner after all sounds fade so nodes don't accumulate
    setTimeout(() => { try { panner.disconnect(); } catch (_) {} }, (sizzleDur + 0.25) * 1000);
}
